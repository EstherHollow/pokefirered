...
void CreateMonWithNature(struct Pokemon *mon, u16 species, u8 level, u8 fixedIV, u8 nature)
{
    ...
    CreatePlayerMon(mon, species, level, fixedIV, 1, personality);
}
...
void CreateMaleMon(struct Pokemon *mon, u16 species, u8 level)
{
    ...
    CreateMon(mon, species, level, 32, 1, personality, otId, VARIANT_DEFAULT, 0);
}

void CreateMonWithIVsPersonality(struct Pokemon *mon, u16 species, u8 level, u32 ivs, u32 personality)
{
    CreatePlayerMon(mon, species, level, 0, 1, personality);
...
{
    CreateMon(mon, species, level, 0, 0, 0, otId, VARIANT_RANDOM, Random32());
...
void CreateBattleTowerMon(struct Pokemon *mon, struct BattleTowerPokemon *src)
{
    ...
    CreateMon(mon, src->species, src->level, 0, 1, src->personality, src->otId, VARIANT_DEFAULT, 0);
...
static void CreateEventLegalMon(struct Pokemon *mon, u16 species, u8 level, u8 fixedIV, u8 hasFixedPersonality, u32 fixedPersonality)
{
    bool32 isEventLegal = TRUE;

    CreatePlayerMon(mon, species, level, fixedIV, hasFixedPersonality, fixedPersonality);
...
static void GiveBoxMonInitialMoveset(struct BoxPokemon *boxMon)
{
    ...
    for (i = 0; learnset[i] != LEVEL_UP_END; i++)
    {
        ...
        moveLevel = (learnset[i] & 0xFE00);

        if (moveLevel > (level << 9))
            break;

        move = (learnset[i] & 0x1FF);
...
u16 MonTryLearningNewMove(struct Pokemon *mon, bool8 firstMove)
{
    ...
    if (firstMove)
    {
        ...
        while ((learnset[sLearningMoveTableID] & 0xFE00) != (level << 9))
        {
    ...
    if ((learnset[sLearningMoveTableID] & 0xFE00) == (level << 9))
    {
        gMoveToLearn = (learnset[sLearningMoveTableID] & 0x1FF);
        sLearningMoveTableID++;
...
void CreateEventLegalEnemyMon(void)
{
    ...
    ZeroEnemyPartyMons();
    CreateEventLegalMon(&gEnemyParty[0], species, level, 32, 0, 0);
    if (itemId)
    {
        u8 heldItem[2];
